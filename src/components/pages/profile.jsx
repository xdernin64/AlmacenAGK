...
const Profile = () => {
    ...
    const renderUserData = () => {
        ...
        if (!user) {
            return <button className="bg-slate-800" onClick={openmodal}>Agregar Datos de perfil</button>;
        }
        return (
            <div>
                <div className="font-bold text-lg pt-2">Codigo:</div>
                <div className="border-b-4 border-gdark text-2xl m-3 text-gdark font-mono">{user.codigo}</div>
                <div className="font-bold text-lg pt-2">Nombres:</div>
                <div className="border-b-4 border-gdark text-2xl m-3 text-gdark font-mono">{user.apellidos + " " + user.nombres }</div>
                <div className="font-bold text-lg pt-2">Celular:</div>
                <div className="border-b-4 border-gdark text-2xl m-3 text-gdark font-mono">{user.numeroCelular}</div>
                <div className="font-bold text-lg pt-2">Labor:</div>
                <div className="border-b-4 border-gdark text-2xl m-3 text-gdark font-mono">{user.labor}</div>
                <div className="font-bold text-lg pt-2">Area:</div>
                <div className="border-b-4 border-gdark text-2xl m-3 text-gdark font-mono">{user.area}</div>
            </div>
        );
    };
